...
**Date:** 2026-04-27
**Version reviewed:** branch `add-mnv-column-haemonc-v1` (commit `e8981b3`)
...
| **A4 — Supply chain** | ✅ Addressed | `requirements.txt` and `requirements-dev.txt` committed — fully pinned lockfiles generated by `pip-compile` from `pyproject.toml`. CI installs from these lockfiles. `pip-audit` runs in CI on every PR and gates on zero CVEs. A CycloneDX SBOM (`sbom.json`) and hashed `requirements-hashed.txt` are generated by the `release-artefacts` CI job on every push to `main` and uploaded as workflow artefacts (90-day retention) for filing in the CRMF. `scripts/generate_release_artefacts.sh` provides local generation. *Remaining governance action: GLH must download and file these artefacts in the CRMF alongside each CSCR.* |
...
| File SBOM and hashed `requirements.txt` from CI workflow artefacts in the CRMF per release | Governance | A4 |
...
| **B3 — Data security** | ⚠️ Partial | TLS is now enforced: `create_engine` passes `sslmode` via `connect_args`, defaulting to `"require"`. Configurable per-environment via `db_credentials.json`. Remaining gaps: no evidence of encryption at rest for the staging DB; error CSVs written to `output_dir` contain workbook filenames which carry specimen IDs; no assertion in CI that PII is absent from logs. |
| **B4 — System security** | ✅ Addressed | `bandit` (SAST) and `pip-audit` (CVE scan) run in a dedicated `security` CI job on every PR, both gating on zero findings. Five B608 false positives in `db.py` suppressed with `# nosec B608` annotations (table names from trusted deployment config; all values parameterised). All dependencies pinned in committed lockfiles (`requirements.txt`, `requirements-dev.txt`). Hashed `requirements-hashed.txt` generated at release time. Python 3.12 — currently supported; no explicit upgrade policy documented. |
...
| **C2 — Proactive security event discovery** | ⚠️ Partial | `pip-audit` in CI provides CVE scanning of dependencies. No penetration testing or threat intelligence process. *Remaining governance control for the GLH.* |
...
| **A4.a** Supply chain | A4 | ✅ Addressed (software); governance action outstanding | Pinned lockfiles committed; `pip-audit` in CI; SBOM and hashed `requirements.txt` generated as CI artefacts per release. Remaining: GLH must download and file these artefacts in the CRMF alongside each CSCR. |
| **B2.a** Identity verification, authentication and authorisation | B2 | ⚠️ Partial | Narrow DB role (`augean_writer`) is evidence of least privilege. Credential file gitignored. `sslmode=require` enforced. Gaps: no access review records, no credential rotation schedule, no operator identity in audit trail. |
| **B4.d** Vulnerability management | B4 | ✅ Addressed | `bandit` (SAST) and `pip-audit` (CVE scan) run in CI on every PR, both gating on zero findings. Pinned lockfiles committed; hashed lockfile generated at release time. |
...
**Current DSPT position:** 2 outcomes fully addressed (software controls complete, governance filing outstanding for A4.a), 4 partially addressed, 2 gaps. Previously: 3 partial, 5 gaps.
...
- [x] `requirements-hashed.txt` with hashes (per release) — generated by `release-artefacts` CI job on push to `main`; download from GitHub Actions and file in CRMF
- [x] SBOM (CycloneDX JSON) — generated by `release-artefacts` CI job alongside hashed requirements; download and file in CRMF
- [x] CVE scan output per release — `pip-audit` runs in CI on every PR; CI log is the evidence; also generated by `scripts/generate_release_artefacts.sh`

**Per-release:**
- [ ] Clinical Safety Case Report (CSCR), CSO-signed, before clinical use
- [x] CI test suite output — tests exist and pass (166 passing, 96% coverage); output not yet filed as formal DCB §6.3 verification evidence
...
- `requirements.txt`, `requirements-dev.txt` → supply chain evidence for A4.a (committed lockfiles)
- `release-artefacts-<sha>` CI artefacts (SBOM + hashed requirements) → A4.a CRMF filing
...
4. Add operator identity to the `staging_workbooks` table for C1 audit logging
5. **File release artefacts in CRMF** — download `release-artefacts-<sha>` from the CI run for each release and file in `CRMF/evidence/release_configs/` alongside the CSCR (satisfies A4.a)
6. Document credential rotation, access review, and operator competency records for B2.a and D1.a
7. Write a data integrity incident response procedure for D1.a
8. Write the IG governance SOP for patient data processing and staging DB access controls (consent/lawful basis, opt-out checking, authorised readers of `testdirectory.inca`) for E2.b and E3.a
