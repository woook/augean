...
This installs all dependencies at the exact pinned versions recorded in `requirements-dev.txt` (generated by `pip-compile` from `pyproject.toml`), then installs the `augean` package itself in editable mode. Editable mode means changes to the source files take effect immediately without reinstalling, and the `augean` CLI command is available in your PATH.
...
`requirements.txt` contains the pinned runtime dependencies only (no dev tools). Using it ensures production runs on the same resolved versions as CI.

### Updating dependencies

When adding or changing a dependency in `pyproject.toml`, regenerate both lockfiles and commit them in the same PR:

```bash
pip-compile pyproject.toml --no-strip-extras --output-file requirements.txt
pip-compile pyproject.toml --extra dev --no-strip-extras --output-file requirements-dev.txt
```
